...
#
#       OpenAlea WebSite : http://vplants.gforge.inria.fr
################################################################################
"""
...
from vplants.image.pil import Image, ImageOps

#~ from inrimage import *
from vplants.image.serial.inrimage import read_inrimage, write_inrimage
#~ from lsm import *
from vplants.image.serial.lsm import read_lsm
#~ from tif import *
from vplants.image.serial.tif import read_tif, write_tif
...
def save (filename, img, is_vectorial=False) :
    ...
    if lenShape==2:
        print "vplants.image.serial.basics.save: assuming 2D scalar image"
        img = img.reshape(img.shape+(1,))
        if len(resolution) == 2:
            resolution += (1.,)
    elif lenShape == 3 and not is_vectorial:
        print "vplants.image.serial.basics.save: assuming 3D scalar image"
    elif lenShape == 3 and  is_vectorial:
        print "vplants.image.serial.basics.save: interpreting as 2D scalar RGB[A] image"
        img = img.reshape( img.shape[:2] + (1, img.shape[2]) )
        if len(resolution) == 2:
            resolution += (1.,)
    elif lenShape == 4 and not is_vectorial:
        print "vplants.image.serial.basics.save: assuming 3D RGB[A] image"
    else:
...
def load (file, mmap_mode=None, is_vectorial=False) :
    ...
    if header == "SpatialImage" :
        ...
        # and we can reshape to a 3D scalar image.
        if len(data.shape) == 2:
            print "vplants.image.serial.basics.load: assuming 2D scalar image"
            data = data.reshape(data.shape+(1,))
            if len(res) == 2:
                res += (1.,)
        elif len(data.shape) == 3 and not is_vectorial:
            print "vplants.image.serial.basics.load: assuming 3D scalar image"
        elif len(data.shape) == 3 and  is_vectorial:
            print "vplants.image.serial.basics.load: interpreting as 2D scalar RGB[A] image"
            data = data.reshape( data.shape[:2] + (1, data.shape[2]) )
            if len(res) == 2:
                res += (1.,)
        elif len(data.shape) == 4 and not is_vectorial:
            print "vplants.image.serial.basics.load: assuming 3D RGB[A] image"
        else:
            raise IOError("Unable to identify image shape and encoding")
